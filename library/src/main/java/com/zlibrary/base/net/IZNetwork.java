...
public interface IZNetwork {
    ...
    String TAG = IZNetwork.class.getSimpleName();

    /**
     * 开始执行网络请求
     *
     * @param reqEntity 请求封装实体
     * @param requestId 请求ID，用于相同实体之间区分不同请求<br/>
     *                  如果两次或多次请求的requestId相同， 框架需要检查之前此requestId的请求是否已经结束<br/>
     *                  如还未执行完成，则不再重复执行<br/>
     *                  如已完成，则继承执行
     * @param callback  执行回调接口
     */
    void request(ZReqEntity reqEntity, int requestId, IZNetworkCallback callback);
     ...
     * @param callback
     */
    void download(String url, String savePath, String saveName, int id, IZNetworkCallback callback);
